...
channel_secret = "b5b96ab1e7a32b8553891eb46af7cdce"
channel_access_token = "x6dZ+AX6rc0si0UAA3KpT3fTSXn6Lk8MYSWkvFig8kIijpa0xyNdYGjZsDN/DLFb9ZGDqlDbCd0dHhDx4e+7AOsIJRbTZlp+5vMBA/X2RPZYYptTiL5lcBjmy9gJlEWccM8dx+Y0HGeatF/SK5SXEQdB04t89/1O/w1cDnyilFU="
...
def handle_join(event):
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text='Joined this ' + event.source.type + 'Hai,Kak! disini astro mau mengasih tau fungsi astro langsung cek aja kak :) '))
...
def message_text(event):
    ...
    if text == 'Kategori':
        ...
        if isinstance(event.source, SourceGroup):
            line_bot_api.reply_message(
                event.reply_token, TextMessage(text='Kakak jahat :('))
            line_bot_api.leave_group(event.source.group_id)
        elif isinstance(event.source, SourceRoom):
            line_bot_api.reply_message(
                event.reply_token, TextMessage(text='Jahat :('))
        ...
        if isinstance(event.source, SourceGroup):
            line_bot_api.reply_message(
                event.reply_token, TextMessage(text='http://line.me/ti/p/OdcxbZg6gQ'))
    elif text == 'tekno':
